#!/bin/sh
# Run after the existing Qwen3.6 session is prepared. The runner keeps its memory and server safety guards.
set -eu
if [ "$#" -eq 1 ]; then
  SESSION=$1
...
status=0
node benchmarks/optimization/runner.mjs run --session "$SESSION" --cli "$CLI" --experiment LLAMA-IQ2-8K-REAL --start-server --max-runs 1 || status=$?
